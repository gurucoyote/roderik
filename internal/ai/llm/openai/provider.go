...
var noopDebugLogger = func(string) {}

func (p *Provider) SetDebugLogger(logger func(string)) {
	if logger == nil {
		p.debugLogger = noopDebugLogger
		return
	}
...
func (p *Provider) debug(msg string, keyvals ...interface{}) {
	if p.debugLogger == nil {
		return
	}

	var builder strings.Builder
	builder.WriteString("[AI][OpenAI] ")
...
func NewProvider(apiKey, baseURL, model, systemPrompt string, maxTokens int) *Provider {
	return &Provider{
...
func (p *Provider) CreateMessage(
	...
	for _, msg := range messages {
		...
		if msg.IsToolResponse() {
			p.debug("processing tool response",
	...
	p.debug("sending messages to OpenAI",
	...
	// Make the API call
	p.debug("openai request config",
		"model", p.model,
		"base_url", p.client.baseURL,
		"final_url", fmt.Sprintf("%s/chat/completions", p.client.baseURL),
	)
	req := CreateRequest{
		Model:    p.model,
		Messages: openaiMessages,
	...
	// Only include max_tokens and temperature if the current model
	// is not in the unsupported list (e.g. GPT-5 and future variants).
	unsupportedModels := map[string]struct{}{
		"gpt-5": {},
	}

	_, unsupported := unsupportedModels[strings.ToLower(p.model)]
	if !unsupported {
		if p.maxTokens > 0 {
...
func (p *Provider) CreateToolResponse(
	...
	p.debug("creating tool response",
