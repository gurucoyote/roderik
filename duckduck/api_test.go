...
func TestSearchLimited_DetectsBotChallenge(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		fmt.Fprint(w, `<html><body><form id="challenge-form"></form><div class="anomaly-modal__modal"></div></body></html>`)
	}))
	defer srv.Close()
	...
	if err == nil {
		t.Fatal("expected an error signaling bot challenge, got nil")
	}
	if !IsChallengeError(err) {
		t.Fatalf("expected IsChallengeError to return true, got err=%v", err)
	}
	if err.Error() == "" {
		t.Fatal("expected a descriptive error message, got empty string")
	}
}
